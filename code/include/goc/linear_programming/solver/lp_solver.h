...
public:
	...
	void ORSolve(Formulation* formulation, const std::unordered_set<LPOption>& options={}) const;

	// Returns: a formulation compatible with the solver.
	static Formulation* NewFormulation();

	static Formulation* NewOrFormulation();

	void Print(Formulation* formulation);
};
} // namespace goc
